...
$document = "# Public API\n\n"
    ...
    . "Run `composer docs` to reject drift. Host adapters retain provider admission, persistence, "
    . "authorization and transactions.\n\n";
$api = json_decode(
    file_get_contents(dirname(__DIR__) . '/resources/public-api/v1.json'),
    true,
    512,
    JSON_THROW_ON_ERROR,
);
foreach ($api['symbols'] as $name => $manifest) {
    ...
    foreach ($type->getProperties(ReflectionProperty::IS_PUBLIC) as $property) {
        ...
        $document .= '### `$' . $property->getName() . "`\n\n```php\npublic "
            ...
            . ($propertyType === null ? '' : (string) $propertyType . ' ')
            . '$' . $property->getName() . ";\n```\n\n";
        $comment = $publicContractDoc($property->getDocComment());
        if ($comment !== '') {
            $document .= "```text\n" . $comment . "\n```\n\n";
        }
    }
    foreach ($type->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
        ...
        foreach ($method->getParameters() as $parameter) {
            ...
            $rendered = ($parameterType === null ? '' : (string) $parameterType . ' ')
